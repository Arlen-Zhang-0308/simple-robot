...
int main(void)
{
  ...
  osThreadDef(CommTask, StartCommTask, osPriorityAboveNormal, 0, 256);
  CommTaskHandle = osThreadCreate(osThread(CommTask), NULL);

  /* definition and creation of MotionTask */
  osThreadDef(MotionTask, StartMotionTask, osPriorityAboveNormal, 0, 256);
  ...
  osThreadDef(DisplayTask, StartDisplayTask, osPriorityBelowNormal, 0, 384);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, OLED_CS_Pin|OLED_RST_Pin|NRF_CSN_Pin, GPIO_PIN_SET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, OLED_DC_Pin|NRF_CE_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : OLED_CS_Pin OLED_DC_Pin OLED_RST_Pin NRF_CSN_Pin
                           NRF_CE_Pin */
  GPIO_InitStruct.Pin = OLED_CS_Pin|OLED_DC_Pin|OLED_RST_Pin|NRF_CSN_Pin
                          |NRF_CE_Pin;
...
void StartDisplayTask(void const * argument)
{
  ...
  {
    display_task_step();
    osDelay(RTOS_DISPLAY_PERIOD_MS);
  }
